...
int main(void){
    ...
    while (1){
        ...
        _bis_SR_register(GIE | LPM0_bits)   //enable maskables 
...
__interrupt void ADC_ISR(void){
    __bic_SR_register_on_exit(LPM0_bits);   //clear bits in mask in the savedstatus  register of an interrupt function
